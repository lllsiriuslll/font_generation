...
def load_global_charset():
    global CN_CHARSET, JP_CHARSET, KR_CHARSET, CN_T_CHARSET, JP_KANA_CHARSET, EN_CHARSET
    cjk = json.load(open(DEFAULT_CHARSET))
    CN_CHARSET = cjk["gbk"]
    JP_CHARSET = cjk["jp"]
    KR_CHARSET = cjk["kr"]
    CN_T_CHARSET = cjk["gb2312_t"]
    JP_KANA_CHARSET = cjk["jp_kana"]
    EN_CHARSET = cjk["en"]
...
def font2img(src, dst, charset, char_size, canvas_size,
    ...
    img_paths = glob.glob(os.path.join(sample_dir, "%d_*.jpg" %(label)))
    if not len(img_paths)==0:
        max_count = 0
        for p in img_paths:
            id = os.path.basename(p).split("_")[1]
            id = int(id.split(".")[0])
            if max_count < id:
                max_count = id
        count = max_count + 1
...
parser.add_argument('--charset', dest='charset', type=str, default='CN',
                    help='charset, can be either: CN, JP, KR, CN_T, JP_KANA, EN or a one line file')
...
if __name__ == "__main__":
    if args.charset in ['CN', 'JP', 'KR', 'CN_T', 'JP_KANA', 'EN']:
