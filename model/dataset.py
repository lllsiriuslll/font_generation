...
from .utils import pad_seq, bytes_to_file, \
    read_split_image, shift_and_resize_image, rotate_image, normalize_image
...
def get_batch_iter(examples, batch_size, augment, rotate_range=0):
    ...
    def process(img):
        ...
        try:
            ...
            if not rotate_range == 0: 
                rotate_angle = random.uniform(-rotate_range, rotate_range)
                img_A = rotate_image(img_A, rotate_angle)
                img_B = rotate_image(img_B, rotate_angle)
...
class TrainDataProvider(object):
    def __init__(self, data_dir, train_name="train.obj", val_name="val.obj", filter_by=None, ignore_label=None, rotate_range=0):
        ...
        if self.ignore_label:
            ...
            self.train.examples = list(filter(lambda e: e[0] not in self.ignore_label, self.train.examples))
            self.val.examples = list(filter(lambda e: e[0] not in self.ignore_label, self.val.examples))
    ...
    def get_train_iter(self, batch_size, shuffle=True):
        ...
        return get_batch_iter(training_examples, batch_size, augment=True, rotate_range=self.rotate_range)
    ...
    def get_val_iter(self, batch_size, shuffle=True):
        ...
        while True:
            val_batch_iter = get_batch_iter(val_examples, batch_size, augment=False, rotate_range=self.rotate_range)
...
class InjectDataProvider(object):
    def __init__(self, obj_path, rotate_range=0):
    ...
    def get_single_embedding_iter(self, batch_size, embedding_id):
        ...
        batch_iter = get_batch_iter(examples, batch_size, augment=False, rotate_range=self.rotate_range)
